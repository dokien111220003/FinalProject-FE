...
export const getProductByNameAndType = async (name, type) => {
  const res = await axios.get(
    `${process.env.REACT_APP_API_URL}/product/get-by-name-and-type?name=${name}&type=${type}`
  );
  return res.data;
};
...
export const getProductsByType = async (type) => {
  const res = await axios.get(
    `${process.env.REACT_APP_API_URL}/product/get-by-type?type=${type}`
  );
  return res.data;
};
